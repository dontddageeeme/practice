...
<%
   int no = Integer.parseInt(request.getParameter("no"));   //*.jsp?no=번호 get방식으로 전달된 번호값을 받아서 정수로 변경하여 저장
   ...
   ResultSet rs = null;            //검색 결과 자료(레코드)를 저장 할 rs
...
<%
   try{   
      ...
      sql = "update guestbook set ghit = ghit+1 where gno=?";      //?는 임의의 모르는 값 번호를 기준으로 조회수 증가
      pstmt = con.prepareStatement(sql);
      pstmt.setInt(1, no);                              //1은 쿼리문의 첫번째 물음표라는 의미, 즉 쿼리문의 첫번째 물음표에 정수로 번호값 저장
      ...
      if (rs.next()){                                    //검색되는 레코드가 하나이기때문에 if문으로 처리
%>
   ...
   <table border = "1">
      ...
      <tr>
         ...
         <%-- 문제) 방명록 글쓰기폼에서 글내용 입력필드 textarea에서 엔터키를 친 부분을 내용보기에서 줄바꿈을 못한다. 이 부분을 해결해보자. --%>
      </tr>
      <tr>
